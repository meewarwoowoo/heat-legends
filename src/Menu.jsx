...
import { menu , results, seasons , cog , drivers , race, standings , cross } from './images';
	...
	return (
		<>
			<nav id="menu">
				<div className="cnt">
					<ul>
						<li className={(props.main==='Race')?'on':'off'} onClick={() => { props.setMain("Race") ; }}><span className="item"><img src={race} /><span className="txt">Race</span></span></li>
						...
						<li className={(props.main==='Config')?'on':'off'} onClick={() => { props.setMain("Config") ; }}><span className="item"><img src={cog} /><span className="txt">Config</span></span></li>
					</ul>
				</div>
			</nav>
			<nav id="subm">
				{ (props.main==='Seasons' || props.main==='Standings' || props.main==='Results' || props.main==='Round') &&
					props.seasonJSON ?
						<div className="cnt">
							<ul className="hdr">
								<li><span className="item" onClick={(e) => { e.currentTarget.parentElement.parentElement.classList.toggle('on') }}><img src={cross} className="on" /><img src={menu} className="off" /><span className="txt">{ props.seasonJSON.year}</span></span></li>
							</ul>
							<ul>
							...
							<ul>
								<li onClick={() => { resetSeasonJSON(props.setSeasonJSON,props.setMain) ; }}><span className="item"><img src={cross} /><span className="txt">Reset</span></span></li>
							</ul>
						</div>
					:null
				}
